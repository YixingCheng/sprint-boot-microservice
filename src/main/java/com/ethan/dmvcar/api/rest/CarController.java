package com.ethan.dmvcar.api.rest;

import com.ethan.dmvcar.entity.Car;
import com.ethan.dmvcar.exception.DataFormatException;
import com.ethan.dmvcar.service.CarService;
...
public class CarController extends AbstractRestHandler{
    ...
    @ApiOperation(value = "Get a paginated list of all carss.", notes = "The list is paginated. You can provide a page number (default 0) and a page size (default 100)")
    public
    @ResponseBody
    ...
    public void updateCar(@ApiParam(value = "The ID of the existing car resource.", required = true)
                            @PathVariable("id") Long id, @RequestBody Car car,
                            HttpServletRequest request, HttpServletResponse response) {
        checkResourceFound(this.carService.getCar(id));
        if (id != car.getId()) throw new DataFormatException("ID doesn't match!");
